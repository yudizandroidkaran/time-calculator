...
class TimeCalculatorController extends GetxController {
  ...
  calculateTime() {
    var completedMinutes =
        (int.parse(completedTimeController.text.split(":")[0]) * 60) +
                int.parse(completedTimeController.text.split(":")[1]) ??
            0;

    if (completedMinutes < 0 || completedMinutes > 480) {
      timeCompletesAt.value =
          "Please go home ASAP, your time has already completed.";
      return;
    }
    var lastEntryMinutes =
        (int.parse(completedTimeAtController.text.split(":")[0]) * 60) +
                int.parse(completedTimeAtController.text.split(":")[1]) ??
            0;
    var remainingWorkMinutes = 480 - completedMinutes;
    var completionTime = lastEntryMinutes + remainingWorkMinutes;
    timeCompletesAt.value =
        "You can go home at ${durationToString(completionTime)}";
  }
  ...
  Future<void> addColon() async {
    if (completedTimeController.text.isNotEmpty &&
        !completedTimeController.text.contains(":")) {
      if (completedTimeController.text.length == 3) {
        completedTimeController.text = "0${completedTimeController.text}";
      }
      completedTimeController.text =
          completedTimeController.text.toString().replaceRange(2, 2, ":");
    }
    if (completedTimeAtController.text.isNotEmpty &&
        !completedTimeAtController.text.contains(":")) {
